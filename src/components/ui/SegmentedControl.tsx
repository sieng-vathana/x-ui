      ...
      {options.map((opt) => {
        ...
        return (
          <button
            ...
            className={cn(
              'inline-flex items-center justify-center gap-1.5 rounded-lg border-0 font-extrabold transition-all duration-200 ease-out',
              size === 'sm' && 'h-9 px-2 text-[11px]',
              size === 'md' && 'h-10 px-3 text-[12px]',
              selected &&
                tone === 'light' &&
          ...
          >
            {opt.icon ? <Icon name={opt.icon} className="text-[15px]" /> : null}
            {opt.label}
          </button>
        )
      })}
